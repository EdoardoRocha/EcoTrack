...
class ProductService {
  ...
  async showAllProducts(data) {
    ...
    const products = await this.productRepository.findAllByPk({
    ...
    const calculatedPrice = price.reduce(
      (acumulator, acctualyValue) => acumulator + acctualyValue,
      0,
    );
  ...
  async purchaseProduct(data) {
    ...
    const { batcheId } = data.body;

    const batche = await this.batchRepository.findByPk(batcheId);
    
    let plainBatche = batche.get({ plain: true });
    
    const quantity = plainBatche.quantity - 1;

    await this.batchRepository.update({ quantity }, { where: { ProductId } });
    return null;
  }
}
